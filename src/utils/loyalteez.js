...
export class LoyalteezClient {
  ...
  async ensureWallet(userEmail) {
    ...
    try {
      const response = await fetch(this.pregenerationEndpoint, {
        ...
        headers: {
          'Content-Type': 'application/json',
          'User-Agent': 'Loyalteez-Discord-Bot/1.0',
        },
        body: JSON.stringify(payload)
      });
  ...
  async sendEvent(eventType, userEmail, metadata = {}) {
    ...
    const payload = {
      ...
      eventType,
      userEmail,
      // Domain is optional - event-handler will derive from brandId if needed
      // Using valid domain format if domain lookup is required
      domain: 'discord.loyalteez.app',
      metadata: {
        platform: 'discord',
    ...
    if (this.eventHandler) {
      try {
        ...
        // Use recognized API hostname so event-handler routes correctly (not as website proxy)
